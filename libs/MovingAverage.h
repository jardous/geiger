...
class MovingAverage {
 public:
  ...
  void _nextIndex() {
    _current_index++;
    if (_current_index >= _array_size) { _current_index = 0; _filled = true; }
  }
 ...
 public:
  ...
  float getAverage() {
    return (float(_average_sum) / (_filled ? _array_size : _current_index));
  }
...
/*
  ...
  */
  size_t size() { return _filled ? _array_size : _current_index ; }
